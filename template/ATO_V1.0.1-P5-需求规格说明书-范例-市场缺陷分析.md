...
> **文档类型**：需求规格说明书（SRS）**黄金范例**，用于约束「迭代需求清单 → SRS」的章节结构、编号层级与写法深度。  
> **编号约定**：`## 1.1` = 一级需求；`### 1.1.x` = 二级需求；`#### 1.1.x.y` = 三级需求（与清单字段对齐）。  
> **需求层级口径（2026-05-13）**：REQ-022～036 同属一级「市场缺陷分析」；其中 **「市场缺陷分析报告」**为二级能力簇（对应 UI「分析报告」Tab 及报告详情），REQ-032～036 为其下三级条目。详见 `docs/prd/V1.0.1-P5/ATO_V1.0.1-P5迭代需求清单.md` 填写说明段。  
> **验收基线**：与已定稿 **`docs/prd/V1.0.1-P5/ATO_V1.0.1-P5-页面需求与交互规格.md`**（下称「页面 PRD」）及工程 Mock 实现不一致时，**以页面 PRD 已定稿条文为准**，本范例在歧义处已显式引用章节号。

### 模块概述

本模块面向 **质量 / 测试 / 研发协同** 角色，在平台内集中查看与管理 **市场缺陷与相关测试缺陷** 数据：从 **RDMS** 同步或刷新数据，在 **可配置列表** 中筛选、检索与就地维护归因字段，进入 **统计报表** 做多维聚合，并可通过 **分析报告任务** 固化结论、导出 Word、发送邮件（首版可为 Mock）。
...
- **数据入口**：定时/手动与 RDMS 对齐（实现可为后端任务 + 列表「刷新」触发增量/批量同步占位）。
- **列表**：按 **创建日期** 年/季/月 + 表头筛选 + 单一搜索框 过滤；支持列设置、行勾选、就地编辑、缺陷 ID 弹层详情、导出。
- **列表 → 报表**：跳转前写入 **`sessionStorage`** 快照（键名与字段见页面 PRD **§7.1 第 4 条**、常量 `MARKET_DEFECTS_LIST_SNAPSHOT_STORAGE_KEY`）；报表统计集与列表过滤结果一致。
- **分析报告**：Tab2 管理任务；详情页 HTML 预览/编辑、分布图块、Word 导出、邮件弹层（与页面 PRD **§3.3 Tab2、§3.5、§1.5** 对齐）。
...
将权威缺陷数据自动同步到平台，避免人工导出与重复统计；支持 **首次当季全量** 与 **之后每日增量 + 未关闭 Bug** 的持续同步，字段与列表展示一致。同步源口径：**RDMS 市场缺陷**、**测试 Bug**（属性 beta-bug，创建部门为系统测试、工厂等——以后台/RDMS 最终定义为准）。

• **用户场景：**

作为质量人员，我希望系统在夜间自动同步 RDMS 缺陷，以便次日上班列表即反映最新未关闭与市场缺陷，而无需手工导入。

• **优先级：** 高

• **输入/前置条件：**

1. 平台与 RDMS 的接口/任务通道已配置（实现期定义）。  
...
1. **正常流程**
   ...
   3. 同步字段集合与列表列 **一致**；新增/变更字段时应有映射版本说明（实现期管理）。
...
2. **异常场景**
   ...
   b) **部分字段映射失败**  
      i. 触发条件：字段缺失、类型不匹配。  
      ii. 系统反馈：该行或该字段占位规则明确（如 `--`），并记录告警。  
      iii. 用户指引：提交缺陷到数据对接负责人。

   c) **首次全量耗时过长**  
      i. 触发条件：当季数据量极大。  
      ii. 系统反馈：任务进度或分批状态可查询（实现期）。  
      iii. 用户指引：避免重复触发全量；关注运维公告。

3. **限制条件**

   a) **测试工具-缺陷分析-市场缺陷列表**  
      i. 影响说明：列表数据依赖同步结果；同步延迟直接影响列表时效性。  
      ii. 关联逻辑：列表为同步数据主消费端。

   b) **无**（本二级不直接修改其它业务域菜单数据）。
...
在定时任务之外，支持用户基于所选范围 **主动触发** 小批量实时查询/同步，缩短等待。
...
1. **正常流程**
   ...
   1. 用户在列表 Tab 勾选一行或多行。  
   2. 点击工具条「刷新」。  
   3. 系统对选中范围发起同步/刷新请求（实现可为 Mock Toast）。  
   4. 成功后更新对应行数据或整表刷新（以后端契约为准；Mock 阶段 Toast 成功即可）。
...
#### 1.1.2.1 列表默认字段展示（REQ-V1.0.1-P5-024）

• **背景与动机：**

统一列表信息结构，支撑分析与复盘；列集合需与 RDMS/业务方最终字段表对齐。
...
1. **正常流程**
   ...
   1. 表格按页面 PRD **§3.3 Tab1「列顺序」** 展示：缺陷 ID、缺陷来源、缺陷标题、创建日期、产品线、是否有效问题、**实际归属团队**（与「缺陷实际归属团队」同义收敛）、**缺陷类型**、主要责任归属、主要责任人、流出原因、优化措施、优化责任人、**完成进度**（0～100，进度条）、自动化是否覆盖、是否可覆盖、不可覆盖原因、自动化未发现原因、操作列等。  
   2. **只读 / 可编辑分区底色**：缺陷 ID～产品线 **浅灰**；是否有效问题～完成进度 **浅蓝**；自动化相关列 **浅绿**（页面 PRD）。  
...
- **迭代需求清单（已更正 REQ-032～036 层级）**：`docs/prd/V1.0.1-P5/ATO_V1.0.1-P5迭代需求清单.md`
