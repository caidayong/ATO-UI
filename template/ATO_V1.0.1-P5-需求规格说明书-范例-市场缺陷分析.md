...
**需求描述结构（固定小节，勿删节）**：各功能点「• **需求描述：**」下须 **依次** 包含 **`1. **正常流程**` → `2. **异常场景**` → `3. **限制条件**`**。若当前能力无独立异常项，**仍须保留 `2. **异常场景**` 标题**，正文可 **留空**（不得整节删除，以免生成/审阅时缺字段）。
...
本二级需求覆盖 **列表展示、配置、过滤、编辑、详情、搜索、导出** 等，对应清单 **REQ-024～029、REQ-031**。其中 **时间维度（年/季/月）与表头筛选** 与页面 PRD **§3.3 Tab1** 对齐。**REQ-029 查询能力** 在本范例中 **收窄** 为「缺陷 ID 精确 + 缺陷描述模糊」且 **优先级低**（见 **§1.1.2.6**）；若与页面 PRD 中「多字段子串」表述不一致，以 **本节 §1.1.2.6** 为准并回写 PRD。
...
1. **正常流程**
   ...
   2. **优化措施**：`Input` 文本，**blur / Enter** 确认行内修改并退出编辑（写入待保存状态，**须与「保存」联动落库**，同下）。  
   3. **完成进度**：数字 + 进度预览，**确定 / Enter** 写入待保存状态并退出；**取消** 放弃本次对该字段的修改（**不提供 Esc 快捷键取消**，与本节下文一致）。  
   4. **`Select` 类可编辑列**：选中选项后 **退出单元格编辑态**（收起下拉）；变更写入 **待保存状态**（行内草稿或与等价前端状态），**在点击列表右上方「保存」之前不向服务端持久化**，详见「限制条件」b）。可选项：**实际归属团队** 来自 **基础数据 · 团队管理**（团队名称）；**主要责任人**、**优化责任人** 来自 **基础数据 · 用户管理**（用户姓名）——**上述两类不在 SRS 中逐项枚举**，开发与验收以 **基础数据源 / 接口返回全量** 为准；Mock 阶段可与 **`src/mocks/data.ts`** 的 `mockTeams`、`mockUsers` 对齐自测。**其余字段** 为业务侧固定或配置枚举，下表第三列给出 **中文枚举**（与 **`src/constants/marketDefectMockOptions.ts`** 及页面 Mock 一致）。  
   ...
   5. **列表右上方「保存」**：用户核对行内修改无误后点击 **保存**，系统 **持久化**（接口对接后）并 **刷新列表**；成功后 `message.success`（Mock 可接受）。**未点击保存前** 不因编辑内容触发「按当前筛选即时重算列表」而导致行被过滤掉（与「限制条件」b）一致）。  
...
3. **限制条件**
   ...
   b) **未点击「保存」前不因编辑触发列表即时重筛**  
      i. 影响说明：用户在表头等筛选条件下查看列表；若在行内修改了会影响筛选匹配的字段（例如表头筛选 **实际归属团队 = S17**，将该行改为 **中台**），**在未点击列表右上方「保存」之前**，列表 **不得** 立即按当前筛选条件重算并 **隐藏/剔除** 该行，以免用户尚未核对就失去了上下文。  
      ii. 关联逻辑：**持久化与列表刷新** 在用户点击 **「保存」** 之后执行；保存成功后再按 **当前时间过滤、表头筛选、搜索** 等条件刷新数据（届时若与筛选不符，行不再出现属预期）。  
      iii. 用户指引：完成多行/多字段编辑后，点击 **「保存」** 做最终确认。

• **补充说明：**

- 可编辑列集合以页面 PRD 为准；清单列出的字段若有增减，以 **字段表评审** 更新。  
- 若已定稿页面 PRD 仍描述为「选中即提交 / 即时落库」，须以 **本节「保存」批确认 + 保存后刷新** 为准回写 PRD，避免与验收冲突。
...
- [ ] 成功/失败反馈明确。  
- [ ] **未保存前**：修改「实际归属团队」等会影响筛选的字段后，在当前表头筛选下 **行仍保留**；点击 **「保存」** 后刷新列表，再按筛选展示。  
...
2. 快照 JSON 字段含：时间维度及 **与列表表头筛选一致** 的筛选项（含 **是否有效问题**、**缺陷归属团队**（若有）、**实际归属团队**、**主要责任归属** 等；以及 `search.text`、`page` / `pageSize`、`updatedAt`（具体键名以页面 PRD **§7.1-4** 与类型定义为准）。
...
| ------ | ---------- | ---- | -------- | ---- | ---- |
| V1.0.1-P5 | 2026-05-13 | 优化 | REQ-029：搜索收窄为缺陷 ID 精确 + 缺陷描述模糊；优先级低 | 与产品口径一致 | §1.1.2.6、§1.1.2 引言 |
| V1.0.1-P5 | 2026-05-13 | 优化 | §1.1.2.4：限制条件「未保存不重筛」；正常流程待保存 +「保存」刷新 | 编辑确认 | §1.1.2.4 |
...
| V1.0.1-P5 | 2026-05-13 | 优化 | §1.1.2.3 / §1.1.3：时间过滤与表头「缺陷归属团队」；快照字段表述对齐 | 与列表口径一致 | §1.1.2.3、§1.1.3 |
