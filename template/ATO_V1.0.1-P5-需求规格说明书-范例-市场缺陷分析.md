...
> **文档类型**：需求规格说明书（SRS）**黄金范例**，用于约束「迭代需求清单 → SRS」的章节结构、编号层级与写法深度，并为 AI 生成与人工审核提供可对齐的范例。
...
**需求层级口径（示例以本稿绑定版本为准）**：若清单中多条 REQ 同属一级「市场缺陷分析」，且「市场缺陷分析报告」等为**二级能力簇**（对应 UI「分析报告」Tab 及报告详情），其下新建/列表/编辑/导出/邮件等为**三级**，须与同版本迭代需求清单 §3 填写说明及列值一致。其它迭代套用本范例时，以**该版本清单**为准替换 REQ 范围与名称。
