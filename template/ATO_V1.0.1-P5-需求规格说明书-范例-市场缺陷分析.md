...
1. **正常流程**
   ...
   4. **`Select` 类可编辑列（选中即提交并退出编辑）**：**实际归属团队** 的可选项来自 **基础数据 · 团队管理**（团队名称）；**主要责任人**、**优化责任人** 来自 **基础数据 · 用户管理**（用户姓名）——**上述两类不在 SRS 中逐项枚举**，开发与验收以 **基础数据源 / 接口返回全量** 为准；Mock 阶段可与 **`src/mocks/data.ts`** 的 `mockTeams`、`mockUsers` 对齐自测。**其余字段** 为业务侧固定或配置枚举，下表第三列给出 **中文枚举**（与 **`src/constants/marketDefectMockOptions.ts`** 及页面 Mock 一致）。  
...
| ------ | ---------- | ---- | -------- | ---- | ---- |
| V1.0.1-P5 | 2026-05-13 | 优化 | §1.1.2.4：团队/用户列不枚举，仅写来源 | 口径简化 | §1.1.2.4 |
