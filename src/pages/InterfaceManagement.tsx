...
import type { ApiCategory, ApiDefinition, ApiMethod, ApiParamRow } from '@/types';
import {
  mockApiCategories,
  mockApiDefinitions,
  mockApiEnvironments,
...
function buildTreeData(
  ...
  return nodes.map((node) => {
    ...
    if (isExpanded) {
      const categoryApis = apis.filter((api) => api.categoryId === node.id);
...
export function InterfaceManagement() {
  ...
  const [selectedTreeKey, setSelectedTreeKey] = useState<string>('cat-env-base');
  const [selectedCategoryId, setSelectedCategoryId] = useState<string>('cat-env-base');
  const [expandedKeys, setExpandedKeys] = useState<string[]>(['root', 'cat-env-base']);
  ...
  const treeData = useMemo(() => {
    return buildTreeData(categories, apis, null, expandedKeys, selectedTreeKey, treeSearch);
  }, [categories, apis, expandedKeys, selectedTreeKey, treeSearch]);
  ...
  const availableParentCategories = useMemo(() => {
    return categories.filter((c) => c.id !== contextMenuNode?.id && c.parentId !== contextMenuNode?.id);
  }, [categories, contextMenuNode]);
